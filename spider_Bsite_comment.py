        ...
        if i != 0:
            try:
                ...
                for j in range(20):
                    ...
                    dict_comment.update({j: dict_temp})
                ...
                # with open("./data/total_comment.json", "a", encoding="utf-8") as f:
                #     f.write('\n')
                #     f.write(comment_data_json)
            except:
                traceback.print_exc()
                continue


    infot = open("./data/total_comment.json", encoding="utf-8").read()
    comment_data_temp = json.loads(infot)
    comment_data_df = pd.read_json(comment_data_temp)
...
def main():
    ...
    # 得到视频评论的json信息，并返回。
    comment_data_json = get_comment_info(oid, 5, headers)  # int(pn)
    a = 1
